...
def remove_range(alist, min, max):
   ...
   *** Important: your code must use comprehensions and should not be more than
   two lines of code including the return statement ***
    """
    return [y for y in alist if not min <=y <= max]
...
def friend_list(friend_dictionary):
    ...
    """
    friends = {}
    for personone, persontwo in friend_dictionary.items():
        if personone not in friends:
            friends[personone] = []
        friends[personone].append(persontwo)
        if persontwo not in friends:
            friends[persontwo] = []
        friends[persontwo].append(personone)
    return friends
    pass
